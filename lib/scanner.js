  ...
  if (pkgJson?.main) {
    ...
    if (!existsSync(mainPath) && !isTsEntry) {
      findings.push(attachFix({ ruleId: "unbuilt-entry", severity: "red",
        desc: `main points to ${pkgJson.main} but file does not exist`, evidence: "package.json" }))
    } else if (isTsEntry && !hasBuild) {
      findings.push(attachFix({ ruleId: "unbuilt-entry", severity: "red",
        desc: `main points to TS source ${pkgJson.main} with no build script (Node cannot run .ts directly)`,
        evidence: "package.json" }))
    }
  }
  ...
  if (pkgJson && !pkgJson.main) {
    ...
    if (!hasEntry) {
      findings.push(attachFix({ ruleId: "no-entry", severity: "red",
        desc: "No main field and no candidate entry file (index.js/lib/index.js etc.); loader fails importing by package name",
        evidence: "package.json" }))
    }
  }
