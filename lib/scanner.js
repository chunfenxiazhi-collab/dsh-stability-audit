...
const FIX = {
  "hook-surface": "Review hook code: ensure interception is necessary and fail-safe; to disable temporarily add disabled: true to the entry in the profile cordis.patch.yml",
  "startup-work": "Move top-level sync scans to lazy/async (e.g. inside a function or after ctx.on('ready')); temporary disable as above",
  "startup-polling": "Review setInterval frequency and cleanup: ensure ctx.effect returns a dispose; increase the interval if needed",
  "many-events": "Merge high-frequency listeners: single dispatcher or subscribe on demand; ensure each ctx.on has a dispose",
  "no-bundle": "Add a dsh.bundle.patch declaration in package.json (e.g. \"dsh\": { \"bundle\": { \"patch\": \"./cordis.patch.yml\" } })",
  "heavy-deps": "Trim dependencies: remove unused packages, move heavy ones to lazy imports/peerDependencies",
  "install-scripts": "Review prepare/postinstall scripts; ensure no network download or unknown code execution; remove scripts and prebuild for release",
  "missing-inject": "Add an export const inject declaration in the entry file (e.g. export const inject = ['tools']); or use ctx.serviceName?. optional access to avoid load-time crashes",
  "unbuilt-entry": "Run the build: cd <plugin-dir> && pnpm install && pnpm build (or the build command in package.json scripts)",
  "dsh-dep-range": "Change @deepseek-ai/* ranges to include the host version (e.g. ^0.1.1-rc.2); or pin with pnpm overrides; for severe cases use a junction to the host global copy",
  "preflight-fail": "Check ~/.dsh/dsh-preflight-report.json for the plugin's error details; fix the config or remove the plugin",
  "missing-dep": "Declare the missing package in package.json dependencies/peerDependencies (version including the host one) then reinstall",
  "no-entry": "Add a main field in package.json pointing to the real entry (or add an index.js entry); build first if no build artifacts exist",
  "exfil": "Remove code sending process.env data to external endpoints (data-exfiltration risk); if reporting is required, tell the user explicitly and sanitize",
  "remote-endpoints": "Review endpoint usage: confirm it is a normal API call; remove suspicious endpoints (non-official domains) or add user confirmation",
}
...
// FIX-1/4 (2026-08-31 battle-test): 解析包入口目标列表：
// main → exports["."]（string 或 {default}）→ dsh.client 声明时 exports["./client"]（client 名册入口）。
// 返回形如 "lib/index.js" 的路径数组（不检查存在性；exports 仅支持 string 与一层 {default} 条件对象）。
export function resolveEntry(pkgJson) {
  if (!pkgJson) return []
  const out = []
  if (typeof pkgJson.main === "string" && pkgJson.main) out.push(pkgJson.main)
  const dot = pkgJson.exports?.["."]
  if (typeof dot === "string") out.push(dot)
  else if (dot && typeof dot === "object") {
    const d = dot.default ?? dot.require ?? dot.import
    if (typeof d === "string") out.push(d)
  }
  if (pkgJson.dsh?.client) {
    const c = pkgJson.exports?.["./client"]
    const cp = typeof c === "string" ? c : (c && typeof c === "object" ? (c.default ?? c.require ?? c.import) : null)
    if (typeof cp === "string") out.push(cp)
  }
  return out
}

// FIX-2 (2026-08-31 battle-test): monorepo workspace 根判定（skin-pack/sshworkspaces 案例）。
// private: true 且（workspaces 字段 或 packages/ 子目录存在）→ 根包不是插件本体。
export function isWorkspaceRoot(pkgJson, dir) {
  if (!pkgJson || pkgJson.private !== true) return false
  if (Array.isArray(pkgJson.workspaces) || (pkgJson.workspaces && typeof pkgJson.workspaces === "object")) return true
  try { return existsSync(path.join(dir, "packages")) } catch { return false }
}
...
// 非服务访问：ctx.on/ctx.emit/ctx.effect/ctx.middleware/ctx.scope/ctx.config/ctx.logger/ctx.router etc.
...
export function auditPlugin({ dir, name, preflight = null, pkg = null, hostPkgVersions = null }) {
  ...
  const findings = []
  // FIX-2: workspace 根不是插件本体（monorepo 惯例），跳过入口/打包/构建类规则
  const wsRoot = isWorkspaceRoot(pkgJson, dir)
  ...
  if (HOOK_RE.test(code)) {
    findings.push(attachFix({ ruleId: "hook-surface", severity: "red",
      desc: "Registers tool hooks/middleware (pre/post-execute intercepts the whole tool chain; barricade incident)",
      evidence: entry?.file || "?" }))
  }
  ...
  if (TOPLEVEL_RE.test(code)) {
    findings.push(attachFix({ ruleId: "startup-work", severity: "red",
      desc: "Top-level sync scan/index at startup (blocks the event loop; codegraph bootstrap incident)",
      evidence: entry?.file || "?" }))
  }
  // ②b 轮询/定时任务（黄：更新检查/心跳常见，注意频率）
  if (POLL_RE.test(code)) {
    findings.push(attachFix({ ruleId: "startup-polling", severity: "yellow",
      desc: "Contains setInterval polling/timers (watch frequency and lifecycle)", evidence: entry?.file || "?" }))
  }
  // ③ 事件监听过多（黄）
  const evCount = (code.match(EVENT_RE) || []).length
  if (evCount >= 10) {
    findings.push(attachFix({ ruleId: "many-events", severity: "yellow",
      desc: `Global event listeners: ${evCount} (high-frequency broadcast events slow every turn)`, evidence: entry?.file || "?" }))
  }
  // ④ 未声明 dsh.bundle（黄）；FIX-4: client 插件经 exports["./client"] 名册激活（官方机制），
  // 有 dsh.client 声明且 client 产物存在 → 激活路径成立，豁免
  const clientEntryFile = pkgJson?.dsh?.client
    ? resolveEntry(pkgJson).find(p => /client/.test(p) && existsSync(path.join(dir, p)))
    : null
  if (pkgJson && !wsRoot && !pkgJson.dsh?.bundle && !clientEntryFile) {
    findings.push(attachFix({ ruleId: "no-bundle", severity: "yellow",
      desc: "No dsh.bundle manifest declared (installed as a plain dep, never activated)", evidence: "package.json" }))
  }
  // ⑤ 依赖过多（黄）
  const depCount = Object.keys(pkgJson?.dependencies || {}).length
  if (depCount > 50) {
    findings.push(attachFix({ ruleId: "heavy-deps", severity: "yellow",
      desc: `${depCount} dependencies (tree pollution / install bloat risk)`, evidence: "package.json" }))
  }
  // ⑥ 安装脚本非构建类（黄）
  if (pkgJson?.scripts) {
    ...
    if (bad.length) {
      findings.push(attachFix({ ruleId: "install-scripts", severity: "yellow",
        desc: `Non-build install scripts: ${bad.map(([k]) => k).join(",")} (npm lifecycle runs arbitrary code)`,
  ...
  if (missing.length) {
    ...
    if (syncMissing.length) {
      findings.push(attachFix({ ruleId: "missing-inject", severity: "red",
        desc: `Load-time sync access: ${syncMissing.join(",")} not declared in inject${injectList.length ? " (current: [" + injectList.join(",") + "] )" : " (no inject declared)"} (boot crash: cannot get property without inject)`,
        evidence: entry?.file || "?" }))
    } else {
      findings.push(attachFix({ ruleId: "missing-inject", severity: "yellow",
        desc: `Deferred access: ${missing.join(",")} not declared in inject${injectList.length ? " (current: [" + injectList.join(",") + "] )" : " (no inject declared)"} (may be undefined at runtime)`,
        evidence: entry?.file || "?" }))
    }
  }

  // ⑦c v0.6: 入口指向未构建源码（红：加载即崩溃，案例 2.2/2.3）
  // FIX-1/3: 入口检查扩展到 exports["."] / exports["./client"]（resolveEntry），命中项记入 entryPath 供 remote npm 对照
  if (pkgJson && !wsRoot) {
    ...
    for (const entryPath of resolveEntry(pkgJson)) {
      const target = path.join(dir, entryPath)
      const isTsEntry = /\.(ts|mts|cts)$/.test(entryPath)
      if (isTsEntry) {
        // TS 入口：文件存在但无构建脚本 → 红（Node 无法直接跑 .ts）；文件不存在 → 跳过（保守，旧语义）
        if (existsSync(target) && !hasBuild) {
          findings.push(attachFix({ ruleId: "unbuilt-entry", severity: "red",
            desc: `Entry ${entryPath} is TS source with no build script (Node cannot run .ts directly)`,
            evidence: "package.json", entryPath }))
          break
        }
      } else if (!existsSync(target)) {
        findings.push(attachFix({ ruleId: "unbuilt-entry", severity: "red",
          desc: `Entry ${entryPath} (from main/exports) points to a missing file`, evidence: "package.json", entryPath }))
        break
      }
    }
  }
  ...
  if (pkgJson?.dependencies && hostPkgVersions) {
    ...
    for (const [dep, range] of Object.entries(pkgJson.dependencies)) {
      ...
      if (!semverSatisfies(hostVer, range)) bad.push(`${dep}@${range} (host ${hostVer}）`)
    }
    if (bad.length) {
      findings.push(attachFix({ ruleId: "dsh-dep-range", severity: "yellow",
        desc: `@deepseek-ai/* dependency range excludes the host synced-package version: ${bad.join(", ")} (pnpm may hoist an old build; tool layer breaks)`,
        evidence: "package.json" }))
    }
  }
  ...
  if (code && pkgJson) {
    ...
    if (missing.size) {
      findings.push(attachFix({ ruleId: "missing-dep", severity: "red",
        desc: `Entry imports not declared in package.json: ${[...missing].join(", ")} (Cannot find package after install)`,
        evidence: entry?.file || "package.json" }))
    }
  }

  // ⑦f v0.9: 无 main 且无候选入口（红：加载器按包名导入失败，verify 案例）
  // FIX-1: resolveEntry 识别 exports["."]/exports["./client"]（empty-fort 案例）
  if (pkgJson && !wsRoot && resolveEntry(pkgJson).length === 0) {
    ...
    if (!hasEntry) {
      findings.push(attachFix({ ruleId: "no-entry", severity: "red",
        desc: "No main/exports entry and no candidate entry file (index.js/lib/index.js etc.); loader fails importing by package name",
        evidence: "package.json" }))
    }
  }
  ...
  if (code) {
    ...
    if (envMatches.length > 0 || sensKeys.size > 0) {
      ...
      findings.push(attachFix({ ruleId: "exfil", severity: "red",
        desc: `Data-exfiltration risk: ${sensKeys.size ? "sensitive env reads " + [...sensKeys].join(",") : "fetch/WebSocket carrying process.env data"} (total ${envMatches.length} occurrences)`,
        evidence: entry?.file || "?",
        endpoints: urls }))
    }
    ...
    if (allEndpoints.length > 0 && !findings.some(f => f.ruleId === "exfil")) {
      const note = wsConnects > 0 ? `; WebSocket connections: ${wsConnects} (call-back risk needs human review)` : ""
      findings.push(attachFix({ ruleId: "remote-endpoints", severity: "yellow",
        desc: `Hardcoded remote endpoints: ${allEndpoints.length}: ${allEndpoints.slice(0, 5).join(", ")}${allEndpoints.length > 5 ? " etc." : ""}${note}`,
        evidence: entry?.file || "?",
  ...
  if (preflight) {
    ...
    if (crit.length) {
      findings.push(attachFix({ ruleId: "preflight-fail", severity: "red",
        desc: `Preflight smoke failed/unresolved: ${crit[0].message?.slice(0, 80)}`,
    ...
    for (const r of results) {
      ...
      if (p?.dir) {
        try {
          ...
          r.dynamic = { status: "skipped", detail: `Dynamic verification unavailable: ${e.message.slice(0, 60)}` }
        }
      }
    }
  }
