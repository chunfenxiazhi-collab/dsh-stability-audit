...
export function dynamicCheck({ target, timeoutMs = 120000, task = "reply OK" }) {
  ...
  try {
    ...
    if (!installOk) {
      const blocked = /allowBuilds|prepare script|git-hosted/i.test(inst.stderr || "")
      return {
        status: "fail",
        installStatus: blocked ? "install-blocked" : "install-failed",
        detail: blocked
          ? "Install blocked: pnpm blocked git-source build scripts (allowBuilds required, unavailable in isolation)"
          : `Install failed (exit ${inst.status}): ${(inst.stderr || "").slice(0, 120)}`,
        installExit: inst.status ?? null, bootExit: null,
    ...
    return {
      ...
      installStatus: "ok",
      detail: bootOk ? "Isolated install passed + no loader errors (headless task needs real credentials)" : `Install ok but boot failed (exit ${bootExit})`,
